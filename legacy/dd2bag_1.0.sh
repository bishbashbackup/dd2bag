...
# Script started 18/04/2024 7:26
...
#			echo "this is content" > "$tempdir"/"$AIP"/"$AIP"_"$count".dd
...
#			read -p "Move files over" wait
			...
			echo "Error: Disk not found. Try again? (Answer yes or no): "
			while true; do
			read choice
			if [[ "$choice" =~ ^(y|Y|yes|Yes|YES)$ ]]; then
				...
				if [ -z $(ls -A "$tempdir/$AIP") ]; then
					...
					echo "moving files from $tempdir/$AIP to $target"
...
if [[ "$premis" == true ]]; then
	...
	ddrescueline=$(grep "End of run" "$ddrescuelogs"/eventlog.txt)
	
	read -r duration percentage _ <<< "$ddrescueline"

	if [[ "$percentage" == "100.00%" ]]; then
	    outcome="success"
	elif [[ "$percentagetest" == "0.00%" ]]; then
	    outcome="failure"
	else
	...
	agentxml+="
		<agent>
			<agentname>$imagername</agentname>
			<agentrole>executing program</agentrole>
		</agent>
		<agent>
			<agentname>$USER</agentname>
			<agentrole>implementer</agentrole>
		</agent>"
	eventxml+="
		<event>
			<eventid>$eventid</eventid>
			<eventdate>$(date -Ins)</eventdate>
			<eventdetail>runtime: "$duration"</eventdetail>
			<eventoutcome>"$outcome"</eventoutcome>
			<eventoutcomedetail>pct rescued: "$percentage"</eventoutcomedetail>
		</event>"
